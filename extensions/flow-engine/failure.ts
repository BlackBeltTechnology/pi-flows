...
export function classifyThrownError(err: unknown): FailureInfo {
  ...
  // Detect FlowHardError by name as well as instanceof: code-node handlers are
  // loaded via dynamic import, so a handler's `FlowHardError` may be a distinct
  // class identity from the engine's. Name matching keeps the hard signal
  // reliable across that module boundary.
  const isHard = err instanceof FlowHardError
    || (err instanceof Error && err.name === "FlowHardError");
  if (isHard) {
