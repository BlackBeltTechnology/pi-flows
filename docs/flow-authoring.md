...
auto_end: true                   # optional — end non-interactive parent session on success (default: false)
...
| `auto_end` | No | `false` | When `true`, gracefully ends the parent pi session once the flow finishes with `success` — but only for non-interactive (headless) sessions. Interactive human sessions are never closed. See [Auto-end behavior](flows.md#auto-end-behavior) in `flows.md`. |
