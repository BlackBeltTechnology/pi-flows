...
Skills: bundles of reference documentation agents query at runtime. Each skill: directory containing index file (`SKILL.md`) and topic-specific Markdown files.
...
Skills provide domain-specific knowledge to agents without bloating system prompts. `SKILL.md` injected into agent's prompt. Topic files accessed on-demand via `skill_read` tool.

**Best practices:**
- Keep `SKILL.md` concise — injected into every agent declaring skill
- Detailed reference material goes in topic files
- One topic per file, under 500 lines
- Code examples in fenced blocks with language tags
- "Best Practices" sections at end of each topic file
...
Extensions: TypeScript modules that hook into pi-coding-agent lifecycle via `ExtensionAPI`.

### Extension Registration

Extensions declared in package manifest:
...
Module must export default function:
...
Typical domain package extension registers agents, flows, skills, custom tools, guards:
...
Agent extensions load into spawned subagent sessions. Enforce sandboxing, register provider middleware, add custom tools:
