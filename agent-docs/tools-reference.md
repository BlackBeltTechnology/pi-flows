...
>
> **Toggle live:** `/flows:edit-mode <on|off>` toggles this. Inbound event `flow:set-edit-mode { enabled: boolean }` does same. Writes `flows.editFlow` to project `.pi/settings.json`, reconciles `flow_agents`/`flow_write`. Command path reloads (live this session). Event path: tools immediate, skill visibility next session. `edit-flow` skill prompt visibility coupled to same toggle: on -> `disable-model-invocation`=false (visible), off -> hidden. See flow-authoring.md, events-api.md.
