...
Guide for building npm packages that plug into pi-flows — registering custom agents, flows, skills, tools, cards, workflows, guards. Well-structured extension package gives domain teams clean way to add project-specific capabilities without forking pi-flows.
...
Declare pi-flows and pi-coding-agent as **peer dependencies** (not regular dependencies). Ensures package uses same instances already loaded by host.
...
`pi.extensions` array tells pi loader which entry points to activate. Paths relative to package root, resolved at runtime.
...
- Scans all `*.md` files in directory (non-recursive, `.chain.md` files excluded).
- Each file parsed as agent definition. See [flow-authoring.md](flow-authoring.md) for `.md` format.
- Agent names come from `name:` frontmatter field.
- **Priority order:** extra packages → pi-flows built-ins → project-local (`.pi/flows/agents/`). Later registrations **overwrite** earlier on name collision.
...
- Flow names derived from filesystem path:
  - `flows/build.yaml` → `build`
  - `flows/my-domain/build.yaml` → `my-domain:build`
- Each flow registered as slash-command (`/my-domain:build`).
...
Each subdirectory must contain `SKILL.md` with `files:` list. Topic files read on-demand via `skill_read` tool.
...
Tools registered here **not** available in main session — only in spawned agent subprocesses. Prevents leaking domain-specific tools to user-facing LLM.

To declare these tools in agent frontmatter, list by name in `tools:`:
```markdown
---
name: my-researcher
...
Cards customize metric display in agent dashboard. `card.metric` frontmatter field on agent selects renderer.
...
Workflows define pipeline stages shown in breadcrumb:
```typescript
pi.events.emit("flow:register-workflow", {
  id: "my-pipeline",
  stages: [
...
Gates block flows from starting when precondition fails. Common use: require authentication before running flows that call authenticated APIs.
...
Gates checked synchronously. `check` function must return synchronously.

### 4g. Guards (agent extensions)

Guards run inside every spawned agent session. Use to enforce sandboxing, add provider middleware, or inject session-specific tools.
...
File must export default function: `export default function(pi: ExtensionAPI) { ... }`

### 4h. Footer segments

Add status indicator to TUI footer bar:
...
After unregistering flows directory, slash-commands for those flows replaced with no-op handlers (no longer do anything).

---

## 6. Observing flow execution

Extension can listen to full flow lifecycle for metrics, logging, or UI updates:
...
See [events-api.md](events-api.md) for full list of observable events.

---

## 7. Using the public API directly

For programmatic flow execution (e.g., from CI tool), import from `pi-flows`:
