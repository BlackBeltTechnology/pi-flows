...
Guide for creating custom pi-flows package from scratch.

## Package Structure

pi-flow package: standard npm package with `pi` manifest:
...
Agents: Markdown files with YAML frontmatter in `agents/` directory:
...
3. **Explicit inputs** — `inputs:` for upstream dynamic data
4. **Clear boundaries** — `access:` patterns prevent collisions
5. **Structured output** — agents call `finish` to submit structured results
...
- Start with `# Title` and brief overview paragraph
...
- "Best Practices" sections at end of each file
- Cross-reference related topic files
...
2. **Test agents individually** by running directly:
   ```
...
4. **Check agent resolution** — verify all agents referenced in flows exist in `agents/` directory.
