...
pi-flows detects when a flow run is interrupted by parent-session close and has no terminal `flow_complete` record. On session resume and on abort-with-no-live-flow, pi-flows drives the orphaned run to a terminal, clearable state by emitting and persisting a reconciliation event that idempotently terminates the run and allows dashboard clients to clear the flow card.
