# post-flow-summary-rendering Specification

## Purpose

Define how the post-flow summary widget renders after a flow completes: the preserved agent cards stay visible above the scannable summary lines, and `alt+o` continues to reach the per-agent detail overlay for deep drill-in.

## Requirements

### Requirement: Post-flow summary renders preserved agent cards

When a flow completes and the post-flow summary widget mounts, the widget SHALL render every preserved agent card from the `lastCards` snapshot in their grid layout, frozen and read-only, above the summary lines. The rendered cards SHALL include all preserved card kinds — subagent, fork, and loop cards alike — and are not filtered by `fr.results`. The cards SHALL reflect the final state captured at flow completion and SHALL NOT update afterward.
