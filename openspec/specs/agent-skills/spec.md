...
## Purpose
Defines how a flow agent's declared `skills:` are resolved and made usable:
advertised in the agent's system prompt via pi's `formatSkillsForPrompt` (name,
description, location) and read on demand with the standard `read` tool
(auto-granted, with skill directories whitelisted into any `access.read`
sandbox). There is no bespoke `skill_read` tool. Also specifies the guard's
read-access enforcement (correct `path` parameter; first-denial allowed-paths
hint).
## Requirements
