...
- **BREAKING (peer deps):** bumped `@earendil-works/pi-coding-agent`, `pi-tui`, and `pi-ai` peer/dev requirements from `^0.74.0` to **`^0.80.0`**. This is also the fix for a TUI-input regression (arrow keys / Esc / Backspace dead in the dashboard navigate mode and agent-detail overlay): the cause was a version skew — `pi-coding-agent@0.80` requires `pi-tui@^0.80`/`pi-ai@^0.80` (version-locked) but stale `pi-tui` (0.75.x) was being resolved, and pi-tui owns key decoding + overlay focus (changed in pi-tui ~0.76.0 and ~0.78.1). Align all three on the 0.80 line. pi-ai 0.80 removed the standalone `getModel` catalog helper; the flow engine's dead tertiary model-resolution fallback was dropped (registry resolution is unchanged).
