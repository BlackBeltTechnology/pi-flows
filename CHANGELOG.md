...
- **Opt-in auto-end for non-interactive flow sessions** (`flow-auto-end` capability). A flow may declare a top-level `auto_end: true` in its `flow.yaml`; when it finishes, the parent pi session is gracefully shut down via `ctx.shutdown()` — but only when all three conditions hold: (1) the flow opted in, (2) the session is **non-interactive** (headless, e.g. an automation- or dashboard-spawned run that fired `flow:run`), and (3) the terminal status is `success` (never on `aborted`, never on `error`). This closes the missing lifecycle gap where automation sessions lingered indefinitely after their flow completed. Interactive human sessions are never closed. There is no settings toggle — the non-interactive gate is the safety boundary. See [docs/flows.md](docs/flows.md#auto-end-behavior) and [docs/flow-authoring.md](docs/flow-authoring.md).
